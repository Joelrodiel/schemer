 ...
 */
inline static std::string run_instruction(std::vector<std::string>& tokens, std::string left, std::string right, std::vector<Symbol*>& symbols, bool DEBUG)
{
    std::string output;
    ...
    else if (tokens[0] == "define")
    {
        ...
        {
            std::vector<std::string> funcParams = tokenize(tokens[1], symbols, true, DEBUG);
        }
    }
    else if (tokens[0] == "eq?")
        output = (left == right) ? "#t" : "#f";
...
inline static std::string execute(std::vector<std::string> tokens, std::vector<Symbol*>& symbols, bool silent, bool DEBUG)
{
    ...
    output = run_instruction(tokens, left, right, symbols, DEBUG);
